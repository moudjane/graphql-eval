...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (!response || !response.data?.register?.token) {
    ...
    successMessage.value = 'Utilisateur créé avec succès. Vous allez être redirigé.'
      ...
      <div class="border-2 border-gray-900 bg-white p-6 font-mono">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div>
            ...
            <input id="confirmPassword" type="password" v-model="confirmPassword" required class="w-full border-2 border-gray-900 p-2 font-mono" />
          </div>
          <p v-if="error" class="text-red-500">{{ error }}</p>
          <p v-if="successMessage" class="text-green-500">{{ successMessage }}</p>
          <button :disabled="!isValid" type="submit" class="border-2 border-gray-900 bg-hn-orange px-6 py-2 font-bold text-white hover:bg-orange-500">
            S'inscrire
          </button>
          <router-link to="/login" class="text-gray-600 hover:text-hn-orange hover:underline">
            Déjà un compte ?
          </router-link>
        </form>
      </div>
